...
//        )
    ...
    private var maxTranslationX = 1000 //
...
//    private lateinit var progressBar: ProgressBar
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        player1 = findViewById(R.id.player1) // 確保你已經在布局中定義了 player1 並對應正確的 ID

        // 計算 maxTranslationX，需要在 player1 的寬度被確定之後計算
        player1.post {
            maxTranslationX = resources.displayMetrics.widthPixels - player1.width
        }
...
//                                    getString(R.string.tfe_pe_tv_cycle, "The ${maxDiffDataList.size}   cycle")
                        ...
                        private fun incrementCorrectCount() {
                            ...
                            private fun updatePlayer1Position() {
                                // 假設每增加一個 correctCount，player1 移動 10 單位
                                val newX = player1.translationX + 10
                                // 檢查 newX 是否超出了屏幕邊界（在這裡使用 maxTranslationX 來模擬）
                                if (newX < maxTranslationX) {
                                    player1.translationX = newX
                                } else {
                                    // 如果超出屏幕，停止增加 correctCount
                                    player1.translationX = maxTranslationX.toFloat() // 確保 player1 不會移出屏幕
                                    // 可以在這裡添加一些代碼來停止 correctCount 的進一步增加
                                }
                            }
